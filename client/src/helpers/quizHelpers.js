...
function chooseCorrectAnswer(arr, ind) {
  const resultObj = {
    option: arr.translations[ind].result,
    optionLang: arr.translations[ind].lang,
    isCorrect: true,
  }
  return resultObj
}
function chooseOtherOptions(array, id) {
  ...
  const result = resultTargetArr.map(elem => {
    ...
    return { option: elem.translations[randomI].result, optionLang: elem.translations[randomI].lang, text: elem.text, isCorrect: false }
  })
  return result
}
...
export const makeQuizArray = (array) => {
  ...
  array.map(item => {
    ...
    const answersArray = [chooseCorrectAnswer(item, randomInd), ...otherOptions]
