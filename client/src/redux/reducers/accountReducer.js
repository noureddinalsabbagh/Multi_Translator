...
const languagesArray = ["de", "es", "ar", "tr", "it", "pt", "jp", "el", "he", "ru"]
const accountReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case 'GET_USERCREDS_SUCCESS':
      const restLangs = languagesArray.filter(item => !action.payload.user.languages.includes(item))
      return {
        ...state,
    ...
    case "DISCARD_LANGUAGES":
      ...
      const updatedLangauges = languages.filter((item, index) => index !== action.payload)
      const discardedLang = languages[action.payload]
      return { ...state, user: { ...state.user, languages: [...updatedLangauges] }, restLangs: [...state.restLangs, discardedLang] }
